/*
	Simple Comment plugin for FAR Manager

	Based on:

	SimpleIndent plugin for FAR Manager
	Copyright (C) Vladimir Panteleev

	Block Comment plugin for FAR Manager
...
#ifdef FAR3
// {52d8eecb-acae-42de-9b2f-f1e909948273}
static const GUID MainGuid = {
		0x52d8eecb, 0xacae, 0x42de, {0x9b, 0x2f, 0xf1, 0xe9, 0x09, 0x94, 0x82, 0x73}
};
#endif
		...
		for (line = (int)ei.BlockStartLine; line < ei.TotalLines; line++) {
			...
			if (!rev)		// Comment
			{
				if (egs.StringLength > 0)
					Info.EditorControl(ECTL_INSERTTEXT, CommentStr);
			} else		// UnComment
			{
